...
df3['Revised US'] = df3['Revised US'].str.replace(' that, I want','that I want')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I want','when I want')
df3['Revised US'] = df3['Revised US'].str.replace(' A ',' a ').replace(' AN ', ' an ')
df3['Revised US'] = df3['Revised US'].str.replace(' I need',', I need')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I need','that I need')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I need','when I need')
df3['Revised US'] = df3['Revised US'].str.replace(' I would like',', I would like')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I would like','that I would like')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I would like','when I would like')
df3['Revised US'] = df3['Revised US'].str.replace(' she would',', she would')
df3['Revised US'] = df3['Revised US'].str.replace(' that, she should','that she should')
df3['Revised US'] = df3['Revised US'].str.replace(' when, she should','when she should')
df3['Revised US'] = df3['Revised US'].str.replace(' I can',', I can')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I can','that I can')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I can','when I can')
df3['Revised US'] = df3['Revised US'].str.replace(' I should',', I should')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I should','that I should')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I should','when I should')
df3['Revised US'] = df3['Revised US'].str.replace(' I am',', I am')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I am','that I am')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I am','when I am')
df3['Revised US'] = df3['Revised US'].str.replace(' I like',', I like')
df3['Revised US'] = df3['Revised US'].str.replace(' that, I like','that I like')
df3['Revised US'] = df3['Revised US'].str.replace(' when, I like','when I like')
...
df3['Revised US'] = df3['Revised US'].str.replace(" IN ", " in ")
df3['Revised US'] = df3['Revised US'].str.replace("' ", " ")
df3['Revised US'] = df3['Revised US'].str.replace("'", " ")
...
# ### 2 roles (role 1 or role 2 / role 1 and role 2)----------------------------------------------------------
#
# # split role at word after coordinating conjunction until punctuation
# # ex: as a CSM or PO who has practioner status, I -> as a CSM or PO / who has practioner status
# role_split_phrase_index = []
# role_split_phrase=[]
#
# # use roles from df_to_SimpleNLG_final_FINAL since these have adverb phrases removed.
# df_to_SimpleNLG_final['role_split'] = df_to_SimpleNLG_final_FINAL['role_phrase'].str.split()
# df4['Role Pos Length No Adverb'] = df_to_SimpleNLG_final_FINAL['role_phrase'].str.split().str.len()
# df_to_SimpleNLG_final['role_phrase pos small'] = df4.apply(lambda row: find_role_phrase_pos(row['Role Pos Length No Adverb'],row['role_phrase pos']), axis=1)
#
# for index, row in df_to_SimpleNLG_final.iterrows():
#     for i in range(0, len(df_to_SimpleNLG_final.loc[index,'role_split'])):
#          if df_to_SimpleNLG_final_FINAL.loc[index,'role_phrase']!='As a default role' and df_to_SimpleNLG_final.loc[index,'role_phrase pos small'].count('CCONJ')==1 and df_to_SimpleNLG_final.loc[index,'role_phrase pos small'][i]=='CCONJ':
#              role_split_phrase_index.append(i)
#              role_split_phrase.append(df_to_SimpleNLG_final.loc[index,'role_split'][:i])
#          else:
#              role_split_phrase.append(None)
#              role_split_phrase_index.append(None)
#
# role_split_phrase_index_Flat = list(flatten(role_split_phrase_index))
# step_role = []
# for index, row in df4.iterrows():
#     step_role.append(len(df_to_SimpleNLG_final.loc[index,'role_split']))
#
# stepz_role = []
# start_role = []
# stepz_role = [sum(step_role[:y]) for y in range(1, len(step_role) + 1)]
# start_role = [m - n for m,n in zip(stepz_role,step_role)]
#
# role_split_phrase_div = []
# role_split_phrase_index_div=[]
# for i in range(0,len(stepz_role)):
#        role_split_phrase_div.append(role_split_phrase[start_role[i]:stepz_role[i]])
#        role_split_phrase_index_div.append(role_split_phrase_index_Flat[start_role[i]:stepz_role[i]])
# df_compound = pd.DataFrame()
#
# df_compound['Role1'] = role_split_phrase_div
# df_compound['Role_All'] = df_to_SimpleNLG_final_FINAL['role_phrase'].copy()
# df_compound['Role_split'] = df_to_SimpleNLG_final['role_split'].copy()
#
# for index, row in df_compound.iterrows():
#    df_compound.loc[index,'Role1'] = list(flatten([x for x in df_compound.loc[index,'Role1'] if x is not None]))
#    df_compound.loc[index,'Role1'] = " ".join(str(x) for x in df_compound.loc[index,'Role1'])
#    df_compound.loc[index,'Role1'] = df_compound.loc[index,'Role1'].replace(' ,','')
#
# df_compound['Role1'] = df_compound['Role1'].fillna('')
#
# df_compound['Role1 Index'] = df_compound['Role1'].str.split().str.len()
#
# for index,row in df_compound.iterrows():
#     if df_compound.loc[index,'Role1 Index']>0:
#         df_compound.loc[index,'Role1 Index'] = int(round(df_compound.loc[index,'Role1 Index']))
#
# # print(df_compound['Role1 Index'].head(50))
#
# def split_role(x,y):
#     if y>0:
#         return x[y+1:]
...
#         return []
#
# df_compound['Role2']= df_compound.apply(lambda row: split_role(row['Role_split'], row['Role1 Index']), axis=1)
#
# df_compound['Role2'] = df_compound['Role2'].apply(' '.join)
#
# for index, row in df_compound.iterrows():
#     if df_compound.loc[index,'Role2'] !='' and df_compound.loc[index,'Role2'][0]!='a':
#         df_compound.loc[index,'Role2'] = "As a " + df_compound.loc[index,'Role2']
#     elif df_compound.loc[index,'Role2'] !='' and df_compound.loc[index,'Role2'][0]=='a':
#         df_compound.loc[index,'Role2'] = "As " + df_compound.loc[index,'Role2']
#
# #Identifying dangling preposition phrases that must be reattached to both role 1 and role 2 for completeness
# #out of scope: CCONJ in prep phrase and prep phrases that start with "when" (already handled)
# #   Type1: role 1 + prep phrase
# #   Type2: role 2 + prep phrase
# # As a public services staff in the preservation conservation	['digital', 'imaging', 'services', 'group']	['ADP', 'DET', 'ADJ', 'NOUN', 'NOUN', 'ADP', 'DET', 'NOUN', 'NOUN', 'CCONJ', 'ADJ', 'NOUN', 'NOUN', 'NOUN']
# # 113 ['maintainer', 'of', 'related', 'services']	['ADP', 'DET', 'NOUN', 'CCONJ', 'NOUN', 'ADP', 'VERB', 'NOUN']
#
# carryover_phrase_index = []
# for index, row in df_to_SimpleNLG_final.iterrows():
#      if df_to_SimpleNLG_final_FINAL.loc[index,'role_phrase']!='As a default role' and df4.loc[index,'role_phrase pos'].count('CCONJ')==1 and df_to_SimpleNLG_final.loc[index,'role_phrase pos small'].count('ADP')>1:
#          carryover_phrase_index.append(df_to_SimpleNLG_final.loc[index,'role_phrase pos small'].index('ADP',2)) #finds second instance of ADP since roles start with ADP
#      else:
#          carryover_phrase_index.append(None)
#
# df_compound['Carryover PP Index'] = carryover_phrase_index
# df_compound['Carryover PP Index'] = df_compound['Carryover PP Index'].fillna('')
# df_compound['CCONJ Index'] = role_split_phrase_index_div
#
# for index,row in df_compound.iterrows():
#     if df_compound.loc[index,'Carryover PP Index']!='':
#         df_compound.loc[index,'Carryover PP Index'] = int(round(df_compound.loc[index,'Carryover PP Index']))
#
# df_compound['CCONJ Index']= df_compound['CCONJ Index'].apply(lambda x: [item for item in x if item is not None])
#
# for index, row in df_compound.iterrows():
#    df_compound.loc[index,'CCONJ Index'] = " ".join(str(x) for x in df_compound.loc[index,'CCONJ Index'])
#    df_compound.loc[index,'CCONJ Index'] = df_compound.loc[index,'CCONJ Index'].replace(' ,','')
#
# df_compound['CCONJ Index'] = df_compound['CCONJ Index'].str.strip().str.rstrip().str.lstrip()
# df_compound['CCONJ Index'] = df_compound['CCONJ Index'].astype(str).astype(int)
# #df_compound['Carryover PP Index'] =df_compound['Carryover PP Index'].astype(str).astype(int)
#
# print(df_compound['Carryover PP Index'][69])
# print(df_compound['CCONJ Index'][69])
# print(df_to_SimpleNLG_final_FINAL['role_phrase'][69])
# print(df_to_SimpleNLG_final['role_phrase pos small'][69])
# print(df_compound['Role1'][69])
# print(df_compound['Role2'][69])
#          # 69
#          # 113
#          # 167
#          # 191
#
# for index, row in df_compound.iterrows():
#    if df_compound.loc[index,'Carryover PP Index']!=None and df_compound.loc[index,'CCONJ Index']!=None and df_compound.loc[index,'CCONJ Index'] < df_compound.loc[index,'Carryover PP Index']:
#        df_compound.loc[index,'Carryover Role PP'] = df_compound.loc[index,'Role_split'][df_compound.loc[index,'Carryover PP Index']:]
#    else:
#       df_compound.loc[index,'Carryover Role PP'] = ''
#
# print(df_compound['Carryover Role PP'][69])

# only extract carryover prep phrases that do NOT contain CCONJ. Else, run the risk of making user stories incomplete
# ex: As a public services staff in the preservation conservation and digital imaging group -->
# if becomes split at CCONJ, the name of the group "preservation conservation and digital imaging group" is deconstructed
# and the resulting user stories no longer make sense.


### 3 roles (role 1 or role 2 or role 3 / role 1 and role 2 and role 3)---------------------------------------------
...
#
# for index, row in df4.iterrows():
#     for i in range(0, len(df4.loc[index,'role_split'])):
#          if df4.loc[index,'role']!='As a default role' and df4.loc[index,'role_phrase pos'].count('CCONJ')==2 and df4.loc[index,'role_phrase pos'][i]=='CCONJ':
#              role_split_phrase_index.append(i)
#              role_split_phrase.append(df4.loc[index,'role_split'][:i])
#          else:
#              role_split_phrase_index.append(None)
#              role_split_phrase.append(None)
#
...
df_compound.to_excel(writer, "compound test")
