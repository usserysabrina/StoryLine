...
#
# #Constant - num_tmpl
tmpl = "As a <role>, I want to <action>, so that <end>"
...
# adding punctuation and additional stop words to list from:
...
df_ambig['filtered_tokens'] = df_ambig['tokenized_text'].apply(lambda x: [item for item in x if item not in stop_words])
...
#
#A1. lexical ambiguity per sentence (discounting stop words)
df_ambig_sent['Sentence Lexical Ambiguity']= df_ambig_sent.apply(lambda x: x.isnull().sum(), axis='columns')
...
b4 = "3. Conceptual Density (CD) Level - 'HIGH' if normalized CD is greater or equal to the user provided threshold: %c; 'MED' if normalized CD is between the user provided threshold and half of the same threshold; LOW otherwise." % int(CD_threshold)
...
b6 = "5. Sentence Lexical Ambiguity Level -'HIGH' if normalized ambiguity is greater or equal to the user provided threshold: %a; 'MED' if normalized CD is between the user provided threshold and half of the same threshold; 'LOW' otherwise." % int(ambig_threshold)
...
text_dupintro = 'User story pairs with similarity scores %d over are presented below. Please review this list for potential dependencies or duplication between user stories.' % int(dup_threshold)
